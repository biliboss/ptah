...
- **v1.7 Streaming text input** — `ptah stream` + `say_stream` MCP tool + incremental chunker
...
- **v1.10.11 ONNX session + afplay quality knobs** — single-threaded ONNX Runtime via daemon env (`OMP_NUM_THREADS=1` + `ORT_NUM_THREADS=1` + `OMP_THREAD_LIMIT=1`; Kokoro@v1.4.2 exposes no `OrtSessionOptions` builder, env is the realistic ship); afplay `pitch_resampling.linear.lpf_order=8` (was 0 default) removes 22050→48000 upsample aliasing; engine master `setGainDb(-3.0)` for -3 dBFS headroom on Dora's stressed vowels; 3 new daemon-wide env knobs (`PTAH_AUDIO_DITHER` / `_LPF_ORDER` / `_HEADROOM_DB`); dither documented as no-op today (engine doesn't expose `dither_mode`)
...
- **v1.10.12 SSML phoneme/sub + cadence tricks** — `<phoneme alphabet="ipa" ph="…">` so agents force IPA pronunciation on brand names (Anthropic, Mistral, Groq, Ollama) via piper's espeak-ng `[[ipa]]` brackets; `<sub alias="…">` rewrites code identifiers like `getConditioningLatents` to "get conditioning latents"; `applyCadenceTricks` wraps the last 3 words of 3+-item enumerations with `<prosody pitch="-10%" rate="slow">`, lifts bullet labels with `<prosody pitch="+5%">`, splices an 80ms pink-noise breath every 2-3 sentences when `PTAH_BREATH_WAV` is set; `--cadence` CLI + `cadence` MCP arg + `cadence` SQLite column; 10-field wire format
- **v1.10.13 Structured logging + worker watchdog** — new `log.zig` module wired to `std.options.logFn`; every `std.log.scoped(.daemon|.worker|.audio|.postfx|.mcp)` call writes ISO 8601 lines to BOTH stderr (launchd compat) and a rotating file at `~/.cache/ptah/daemon.log` (3 backups, 10 MiB cap); runtime knobs `PTAH_LOG_LEVEL` / `_SCOPES` / `_PATH` / `_MAX_BYTES` filter without rebuild; diagnosed the v1.10.12 stall — postfx `apply()` did serial stdin write + stdout drain, so any PCM > 64 KiB kernel-pipe-buffer deadlocked when ffmpeg's own output pipe filled; fix: drain thread runs concurrently with the write, plus a watchdog thread `SIGTERM`s the subprocess after `PTAH_POSTFX_TIMEOUT_MS` (default 5000); `workerLoop` gained `defer finishPlaying` so the row flips to `done` even on a panicking sub-call; 10 s debug heartbeat proves the worker thread is alive

See the [Changelog](/ptah/changelog/) for measurements + honest scope per version. The next slate (v1.11+) is unscheduled.

Vote, watch, or send a PR at [biliboss/ptah](https://github.com/biliboss/ptah).
...
- 👍 [GitHub issues with the `roadmap` label](https://github.com/biliboss/ptah/labels/roadmap)
- ⭐ Star the repo — repo traffic guides priority
- 🛠 Send a PR — the changelog will name you

See also: [Roadmap](/ptah/roadmap/), [Changelog](/ptah/changelog/), [TTS engine](/ptah/motor/), [MCP server](/ptah/mcp/), [Playground](/ptah/playground/), [Menubar UI](/ptah/menubar/).
